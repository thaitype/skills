...
# Sync Git
## Usage
Run the sync script from the repo root. The script may be installed at the
project level or the user level, so resolve the path before running (project
...
Then re-run the sync script from the main repo (resolve the path as in **Usage**).
